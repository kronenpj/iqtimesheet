 ...
 */
public class TimeSheetActivityOld extends ListActivity {
	...
	void setupDB() {
		...
		try {
			...
			Toast.makeText(getApplicationContext(),
					e.toString() + " - Exiting", Toast.LENGTH_LONG).show();
			finish();
		}
	...
	void fillData() {
		...
		tasksList.setAdapter(new MyArrayAdapter<String>(
	...
	void checkBoundary() {
		...
		// Want to know whether the last entry follows: timeIn < boundary <
		// timeOut
		// TODO: Should this be something more interactive?
		if (timeIn < boundary && boundary < timeOut) {
			Toast.makeText(getApplicationContext(),
					"checkBoundary: Found entry.", Toast.LENGTH_LONG).show();
			Log.d(TAG, "checkBoundary: Clipping: row " + lastRowID
					+ ", timeOut: " + timeOut);
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		optionsMenu = menu;
		item.setIcon(R.drawable.ic_menu_info_details);
		item = menu.add(0, MenuItems.SCRUB_DB.ordinal(), 10,
				R.string.scrub_database);
		item.setIcon(R.drawable.ic_menu_preferences);
		return true;
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		if (item.getItemId() == MenuItems.NEW_TASK.ordinal()) {
			intent = new Intent(getApplicationContext(), AddTaskHandler.class);
			try {
		...
		// intent = new Intent(getApplicationContext(), DayReport.class);
		// try {
		...
		// intent = new Intent(getApplicationContext(), WeekReport.class);
		// try {
		// startActivityForResult(intent, ActivityCodes.REPORT.ordinal());
		// } catch (RuntimeException e) {
		// Log.e(TAG, "RuntimeException caught in "
		...
		if (item.getItemId() == MenuItems.ABOUT.ordinal()) {
			intent = new Intent(getApplicationContext(), AboutDialog.class);
			try {
		...
		if (item.getItemId() == MenuItems.SCRUB_DB.ordinal()) {
			...
			while (!taskCursor.isAfterLast()) {
				...
				long boundary = TimeHelpers.millisToEoDBoundary(timeIn,
						prefs.getTimeZone()) - 1000;

				long thisRowID = taskCursor.getPosition();
				// Want to know whether the last entry follows: timeIn <
				// boundary < timeOut
				// TODO: Should this be something more interactive?
				if (timeOut > 0) {
					if (timeIn < boundary && boundary < timeOut) {
						...
						Log.d(TAG,
								"--> boundary: "
										+ TimeHelpers
												.millisToTimeDate(boundary));
						// Toast.makeText(getApplicationContext(),
						// "ScrubDB: Found entry: " + lastRowID,
						// Toast.LENGTH_LONG).show();
				...
				if ((timeOut == 0) && (thisRowID < lastDBentry)) {
					...
					Log.d(TAG,
							"--> with time: "
									+ TimeHelpers.millisToTimeDate(boundary));
					db.updateEntry(thisRowID, -1, null, timeIn, boundary);
					count++;
				}
				taskCursor.moveToNext();
			...
			Toast.makeText(getApplicationContext(),
					"ScrubDB: Fixed " + count + " entries.", Toast.LENGTH_LONG)
					.show();
			return true;
		}
		return false;
