...
public class TimeSheetActivityBasic extends
	...
	public void test00Preconditions() {
		...
		mView = (ListView) mActivity
				.findViewById(com.googlecode.iqapps.IQTimeSheet.R.id.tasklistfragment);
		assertNotNull(mView);
	}
	...
	public void test40EditTaskStart() {
		...
		if (whatHour != 23) {
			View btn = solo.getView(com.googlecode.iqapps.IQTimeSheet.R.id.starttime);
			solo.clickOnView(btn);
		} else {
			View btn = solo.getView(com.googlecode.iqapps.IQTimeSheet.R.id.endtime);
			solo.clickOnView(btn);
		}
		solo.sleep(SLEEPTIME);
	...
	private void setAlignTimePreferenceViaMenu(int downCount) {
		...
		solo.clickOnText(solo
				.getString(com.googlecode.iqapps.IQTimeSheet.R.string.align_minutes_title));
		solo.sleep(SLEEPTIME);
		while (solo.scrollUpList(0))
			;

		String[] durations = getActivity().getResources().getStringArray(
				com.googlecode.iqapps.IQTimeSheet.R.array.alignminutetext);
